...
public static class MapExtensions
{
    ...
    //public static async Task<byte[]?> GetImageAsync( this IStaticFragment fragment, CancellationToken ctx = default )
    //{
    //    Logger?.Verbose("Beginning image retrieval from web");

    //    var request = fragment.Projection.CreateMessage(fragment, fragment.Projection.Scale);
    //    if (request == null)
    //    {
    //        Logger?.Error<string>("Could not create HttpRequestMessage for mapFragment ({0})", fragment.FragmentId);
    //        return null;
