    ...
    where TViewport : INormalizedViewport
{
    ...
    //protected byte[]? GetImage( TFrag fragment )
    //{
    //    return Task.Run( async () => await GetImageAsync( fragment ) ).Result;
    //}

    //protected async Task<byte[]?> GetImageAsync(TFrag fragment, CancellationToken ctx = default)
    //{
    //    Logger.Verbose("Beginning image retrieval from web");

    //    var request = CreateMessage(fragment, Scale);
    //    if (request == null)
    //    {
    ...
    //    return await ExtractImageStreamAsync(response, ctx);
    //}
